...
func main(){
	...
	for {
		...
		if (mouse.Buttons & 0x01) == 0x01 {
			// 左クリック検知
			x := int(mouse.Point.X / 31)
			y := int(mouse.Point.Y / 31)
			if x < MASU && y < MASU {
				if block == false {
					block = true
					go game.startTurn(y, x)
					block = false
				} else {
					println("AI思考中・・・")
				}
...
func initGame() (*Games){
	...
	game.pointmap.data = [8][8]int8{
		...
		[8]int8{-12,-15, -3, -3, -3, -3,-15,-12},
		[8]int8{ 30,-12,  0, -1, -1,  0,-12, 30}}
	game.aroundmap = [2][8]int8{
		[8]int8{ -1,  0,  1, -1,  1, -1,  0,  1},
		[8]int8{ -1, -1, -1,  0,  0,  1,  1,  1}}
	game.aroundmapd = [2][4]byte{
		[4]byte{ 0, 0, 7, 7},
		[4]byte{ 0, 7, 0, 7}}
	game.nextstep = [4][2]byte{
		[2]byte{ 1, 2},
		[2]byte{ 1, 4},
		[2]byte{ 3, 2},
		[2]byte{ 3, 4}}
...
func (game *Games) stoneCheck(i, j int, y, x int8) (bool){
	i += y
	j += x
...
func (game *Games) hanten(i, j, y, x int8) (int){
...
func (game *Games) decisionCount(y, x, types byte) (int){
